...
exports.feedbackService = {
    ...
    async list({ page = 1, limit = 10, status, category }) {
        let where = [];
        let params = [];
        let idx = 1;
        if (status) {
    ...
    async detail(id) {
        const feedback = await (0, db_1.query)(`SELECT * FROM phan_anh WHERE id = $1`, [id]);
        if (!feedback.rows[0])
            return null;
        const reporters = await (0, db_1.query)(`SELECT u.id, u."fullName" FROM phan_anh_nguoi pan 
       JOIN users u ON pan."nguoiPhanAnhId" = u.id 
       WHERE pan."phanAnhId" = $1`, [id]);
        return {
            ...feedback.rows[0],
    ...
    async addResponse(id, responder_unit, response_content) {
    ...
    `;
        try {
            const result = await (0, db_1.query)(sql, [formattedResponse, id]);
    ...
    async notify(id, message) {
        ...
        for (const r of reporters.rows) {
            await (0, db_1.query)(`INSERT INTO phan_anh_thong_bao ("phanAnhId", "nguoiPhanAnhId", "noiDung", "ngayTao", "daDoc") 
         VALUES ($1, $2, $3, NOW(), false)`, [id, r.nguoiPhanAnhId, message]);
        }
        return true;
    },
};
