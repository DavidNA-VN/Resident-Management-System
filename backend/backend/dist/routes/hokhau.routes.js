...
const withUserContext = async (userId, fn) => {
 ...
 */
router.get("/ho-khau", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        ...
        const r = await (0, db_1.query)(queryStr, params);
 ...
 */
router.get("/ho-khau/:id", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        const hoKhauId = Number(req.params.id);
        const r = await (0, db_1.query)(`SELECT * FROM ho_khau WHERE id = $1`, [hoKhauId]);
        if ((r?.rowCount ?? 0) === 0) {
 ...
 */
router.post("/ho-khau", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        const { diaChi, tinhThanh, quanHuyen, phuongXa, duongPho, soNha, diaChiDayDu, ngayCap, ghiChu, } = req.body;
        await withUserContext(req.user.id, async (client) => {
            ...
            const r = await client.query(`INSERT INTO ho_khau
 ...
 */
router.patch("/ho-khau/:id", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        await withUserContext(req.user.id, async (client) => {
            const hoKhauId = Number(req.params.id);
            if (!hoKhauId) {
            ...
            const { soHoKhau, diaChi, tinhThanh, quanHuyen, phuongXa, duongPho, soNha, diaChiDayDu, ngayCap, ghiChu, } = req.body;
            if (soHoKhau !== undefined && soHoKhau.trim() !== "") {
                const existing = await client.query(`SELECT id FROM ho_khau WHERE "soHoKhau" = $1 AND id != $2`, [soHoKhau.trim(), hoKhauId]);
            ...
            if (soNha !== undefined)
                fields.push({ column: "soNha", value: soNha });
            ...
            const r = await client.query(`UPDATE ho_khau
           SET ${setClauses}
           WHERE id = $${fields.length + 1}
           RETURNING *`, [...values, hoKhauId]);
            if (r.rowCount === 0) {
                return res.status(404).json({
                    success: false,
                    error: { code: "NOT_FOUND", message: "Hộ khẩu không tồn tại" },
 ...
 */
router.patch("/ho-khau/:id/activate", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        await withUserContext(req.user.id, async (client) => {
            const hoKhauId = Number(req.params.id);
            const { chuHoId, soHoKhau } = req.body;
            if (!hoKhauId || !chuHoId) {
                return res.status(400).json({
                    success: false,
                    error: {
            ...
            if (soHoKhau !== undefined && soHoKhau.trim() !== "") {
                const existing = await client.query(`SELECT id FROM ho_khau WHERE "soHoKhau" = $1`, [soHoKhau.trim()]);
                if ((existing?.rowCount ?? 0) > 0) {
                    return res.status(409).json({
                        success: false,
                        error: {
            ...
            const check = await client.query(`SELECT id, "quanHe" FROM nhan_khau WHERE id = $1 AND "hoKhauId" = $2`, [chuHoId, hoKhauId]);
            ...
            const existingChuHo = await client.query(`SELECT id FROM nhan_khau 
           WHERE "hoKhauId" = $1 AND "quanHe" = 'chu_ho' AND id != $2`, [hoKhauId, chuHoId]);
 ...
 */
router.patch("/ho-khau/:id/change-chu-ho", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
        await withUserContext(req.user.id, async (client) => {
            const hoKhauId = Number(req.params.id);
            const { newChuHoId, oldChuHoNewQuanHe } = req.body;
            ...
            try {
                const newChuHoCheck = await client.query(`SELECT id, "quanHe" FROM nhan_khau WHERE id = $1 AND "hoKhauId" = $2`, [newChuHoId, hoKhauId]);
                ...
                const currentChuHo = await client.query(`SELECT nk.id, nk."quanHe" 
             FROM nhan_khau nk
             INNER JOIN ho_khau hk ON hk."chuHoId" = nk.id
             WHERE hk.id = $1`, [hoKhauId]);
                let oldChuHoId = null;
                if ((currentChuHo?.rowCount ?? 0) === 0) {
                    const chuHoByQuanHe = await client.query(`SELECT id FROM nhan_khau 
               WHERE "hoKhauId" = $1 AND "quanHe" = 'chu_ho' AND id != $2`, [hoKhauId, newChuHoId]);
                ...
                if (oldChuHoId) {
                    ...
                    await client.query(`UPDATE nhan_khau SET "quanHe" = $1 WHERE id = $2`, [newQuanHe, oldChuHoId]);
                }
                await client.query(`UPDATE nhan_khau SET "quanHe" = 'chu_ho' WHERE id = $1`, [newChuHoId]);
                await client.query(`UPDATE ho_khau SET "chuHoId" = $1 WHERE id = $2`, [newChuHoId, hoKhauId]);
                const updatedHoKhau = await client.query(`SELECT * FROM ho_khau WHERE id = $1`, [hoKhauId]);
...
router.get("/ho-khau/:id/history", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireTask)("hokhau_nhankhau"), async (req, res, next) => {
    try {
