...
function extractCccdCandidates(type, payload) {
    const candidates = [];
...
async function buildPrecheckWarnings(opts) {
    ...
    for (const cccd of cccds) {
        const existed = await (0, db_1.query)(`SELECT id, "hoKhauId", "hoTen" FROM nhan_khau WHERE cccd = $1 LIMIT 5`, [cccd]);
    ...
    const quanHe = String(payload?.person?.quanHe || payload?.nhanKhau?.quanHe || "").toLowerCase();
    if (quanHe === "chu_ho" && targetHouseholdId) {
        const hasChuHo = await (0, db_1.query)(`SELECT id FROM nhan_khau WHERE "hoKhauId" = $1 AND LOWER("quanHe") = 'chu_ho' LIMIT 1`, [targetHouseholdId]);
...
(function (RequestType) {
    RequestType["ADD_PERSON"] = "ADD_PERSON";
    RequestType["ADD_NEWBORN"] = "ADD_NEWBORN";
    RequestType["UPDATE_PERSON"] = "UPDATE_PERSON";
    RequestType["REMOVE_PERSON"] = "REMOVE_PERSON";
    RequestType["CHANGE_HEAD"] = "CHANGE_HEAD";
    RequestType["UPDATE_HOUSEHOLD"] = "UPDATE_HOUSEHOLD";
    RequestType["SPLIT_HOUSEHOLD"] = "SPLIT_HOUSEHOLD";
    RequestType["TEMPORARY_RESIDENCE"] = "TEMPORARY_RESIDENCE";
    RequestType["TEMPORARY_ABSENCE"] = "TEMPORARY_ABSENCE";
    RequestType["MOVE_OUT"] = "MOVE_OUT";
    RequestType["DECEASED"] = "DECEASED";
...
async function assertHouseholdHead(userId) {
    const info = await (0, db_1.query)(`SELECT u."personId", nk."hoKhauId", hk."chuHoId"
     FROM users u
...
async function ensurePersonInHousehold(personId, householdId) {
    const person = await (0, db_1.query)(`SELECT id, "hoKhauId", "trangThai" FROM nhan_khau WHERE id = $1`, [personId]);
 ...
 */
router.post("/requests", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireRole)(["nguoi_dan"]), async (req, res, next) => {
    try {
        ...
        try {
            ...
            return res.status(err.status || 403).json({
                ...
                error: {
                    ...
                    message: err.message || "Chỉ chủ hộ đã liên kết mới được phép tạo yêu cầu",
                },
            });
        }
        let finalTargetHouseholdId = targetHouseholdId
            ? Number(targetHouseholdId)
            : null;
        let finalTargetPersonId = targetPersonId
        ...
        try {
            ...
            console.log("[POST /requests] payload (raw):", typeof req.body.payload === "string"
                ? req.body.payload
        ...
        if (type === RequestType.ADD_NEWBORN) {
        ...
        else if (type === RequestType.TEMPORARY_RESIDENCE ||
            type === "TAM_TRU") {
            ...
            const household = await (0, db_1.query)(`SELECT "soHoKhau" FROM ho_khau WHERE id = $1`, [headContext.householdId]);
            ...
            const residenceData = payload?.residence && typeof payload.residence === "object"
                ? payload.residence
                : payload;
            ...
            const nhanKhauId = payload?.nhanKhauId || payload?.residence?.nhanKhauId || null;
            if (nhanKhauId) {
                try {
        ...
        else if (type === RequestType.TEMPORARY_ABSENCE ||
            type === "TAM_VANG") {
            // Default target household to current head's household
            finalTargetHouseholdId = headContext.householdId;
            validationError = validateTemporaryAbsencePayload(payload);
            const nhanKhauId = payload?.nhanKhauId || payload?.absence?.nhanKhauId || null;
            if (nhanKhauId) {
                try {
                    await ensurePersonInHousehold(Number(nhanKhauId), headContext.householdId);
                }
            ...
            const selectedIds = payload?.selectedNhanKhauIds || [];
            if (selectedIds.length > 0) {
                const members = await (0, db_1.query)(`SELECT id FROM nhan_khau WHERE "hoKhauId" = $1 AND id = ANY($2::int[])`, [headContext.householdId, selectedIds]);
        ...
        // Create request
        const result = await (0, db_1.query)(`INSERT INTO requests ("requesterUserId", type, "targetHouseholdId", "targetPersonId", payload)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [
            userId,
            type,
 ...
 */
router.get("/tam-tru-vang/requests", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const { type, fromDate, toDate, keyword, page = "1", limit = "50", } = req.query;
    ...
    `;
        const requestRows = (await (0, db_1.query)(selectQuery, params)).rows;
        const tamTruVangFilters = [];
        let ttvWhere = "WHERE 1=1";
        if (type && String(type).toLowerCase() !== "all") {
            ttvWhere += " AND ttv.loai = $" + (tamTruVangFilters.length + 1);
            tamTruVangFilters.push(String(type).toUpperCase().includes("VANG") ? "tam_vang" : "tam_tru");
        }
        if (fromDate) {
            ttvWhere +=
        ...
        if (keyword && String(keyword).trim()) {
            const kw = `%${String(keyword).trim()}%`;
            ttvWhere += ` AND (nk."hoTen" ILIKE $${tamTruVangFilters.length + 1} OR nk.cccd ILIKE $${tamTruVangFilters.length + 1} OR hk."soHoKhau" ILIKE $${tamTruVangFilters.length + 1} OR hk."diaChi" ILIKE $${tamTruVangFilters.length + 1})`;
            tamTruVangFilters.push(kw);
        }
        // Chỉ lấy những nhân khẩu đang có trạng thái tạm trú/tạm vắng
        ttvWhere += ` AND nk."trangThai" IN ($${tamTruVangFilters.length + 1}, $${tamTruVangFilters.length + 2})`;
      ...
      `;
        const ttvRows = (await (0, db_1.query)(ttvQuery, tamTruVangFilters)).rows;
        ...
        const statusRank = (s) => {
            ...
            if (key.includes("PENDING") || key.includes("CHO"))
                return 3;
            if (key.includes("APPROVED") || key.includes("DA_DUYET"))
                return 2;
            if (key.includes("REJECTED") || key.includes("TU_CHOI"))
                return 1;
            return 0;
        };
        ...
        for (const it of mapped) {
            ...
            const preferNext = nextRank > existingRank ||
        ...
        const items = Array.from(dedupMap.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
 ...
 */
router.post("/tam-tru-vang/requests/:id/approve", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        ...
        const requestResult = await (0, db_1.query)(`SELECT * FROM requests WHERE id = $1`, [requestId]);
        ...
        let nhanKhauId = parsedPayload?.nhanKhauId
            ...
            : null;
        const personPayload = parsedPayload?.person || parsedPayload?.nhanKhau || null;
        if (!tuNgay || !lyDo) {
            return res.status(400).json({
                success: false,
                error: {
        ...
        try {
            ...
            if (!nhanKhauId && personPayload) {
                ...
                const requesterUserId = reqRow.requesterUserId ||
                    reqRow.requesteruserid ||
                    reqRow.requesterUserID;
                const requesterHousehold = await (0, db_1.query)(`SELECT nk."hoKhauId" as "hoKhauId"
             FROM users u
             JOIN nhan_khau nk ON u."personId" = nk.id
             WHERE u.id = $1
             LIMIT 1`, [requesterUserId]);
                if ((requesterHousehold?.rowCount ?? 0) === 0) {
                    ...
                    return res.status(400).json({
                        ...
                        error: {
                            code: "VALIDATION_ERROR",
                            message: "Không xác định được hộ khẩu của người gửi (tài khoản chưa liên kết nhân khẩu)",
                        },
                    });
                }
                ...
                if (!hoTen || !ngaySinh || !gioiTinh || !noiSinh) {
                    ...
                    return res.status(400).json({
                        ...
                        error: {
                            code: "VALIDATION_ERROR",
                            message: "Thiếu thông tin nhân khẩu bắt buộc: hoTen, ngaySinh, gioiTinh, noiSinh",
                        },
                    });
                }
                // If CCCD provided and exists, reuse only if same household
                if (cccd) {
                    const existing = await (0, db_1.query)(`SELECT id, "hoKhauId" FROM nhan_khau WHERE cccd = $1 LIMIT 1`, [cccd]);
                ...
                if (!nhanKhauId) {
                    ...
                    const processedLyDoKhongCoCCCD = !normalizedCccd
                        ? String(lyDoKhongCoCCCD || personPayload?.ghiChu || "").trim() ||
                            null
                        : null;
                    const processedGhiChu = normalizedCccd || lyDoKhongCoCCCD
                ...
                await (0, db_1.query)(`UPDATE requests SET "targetPersonId" = $1 WHERE id = $2`, [nhanKhauId, requestId]);
            }
            // Check if tam_tru_vang record already exists (created by citizen)
            const existingTamTruVang = await (0, db_1.query)(`SELECT id FROM tam_tru_vang WHERE "nhanKhauId" = $1 AND "tuNgay" = $2 AND "trangThai" = 'cho_duyet'`, [nhanKhauId, tuNgay]);
            if ((existingTamTruVang?.rowCount ?? 0) > 0) {
                // Update existing record
                await (0, db_1.query)(`UPDATE tam_tru_vang SET "trangThai" = 'da_duyet', "nguoiDuyet" = $1 WHERE id = $2`, [reviewerId, existingTamTruVang.rows[0].id]);
            }
            else {
                // Fallback: create new record (for legacy requests)
                const loai = reqRow.type === "TAM_TRU" || reqRow.type === "TEMPORARY_RESIDENCE"
                    ? "tam_tru"
                    : "tam_vang";
                await (0, db_1.query)(`INSERT INTO tam_tru_vang ("nhanKhauId", loai, "tuNgay", "denNgay", "lyDo", "nguoiDangKy", "nguoiDuyet", "trangThai")
           VALUES ($1, $2, $3, $4, $5, $6, $7, 'da_duyet')`, [
            ...
            const newStatus = reqRow.type === "TAM_TRU" || reqRow.type === "TEMPORARY_RESIDENCE"
                ? "tam_tru"
                : "tam_vang";
            await (0, db_1.query)(`UPDATE nhan_khau SET "trangThai" = $1 WHERE id = $2`, [
                newStatus,
                nhanKhauId,
            ]);
            ...
            if (diaChi) {
                try {
                    ...
                    await (0, db_1.query)(`UPDATE requests SET payload = $1 WHERE id = $2`, [
            ...
            await (0, db_1.query)(`UPDATE requests SET status = 'APPROVED', "reviewedBy" = $1, "reviewedAt" = CURRENT_TIMESTAMP WHERE id = $2`, [reviewerId, requestId]);
 ...
 */
router.get("/tam-tru-vang/requests/:id", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const requestId = Number(req.params.id);
        if (!requestId) {
            return res.status(400).json({
        ...
        const typeToLoai = (t) => {
            ...
            if (s.includes("ABSENCE") || s.includes("VANG"))
                return "tam_vang";
            if (s.includes("RESIDENCE") || s.includes("TRU"))
                return "tam_tru";
            return null;
        };
        ...
        WHERE r.id = $1 AND r.type IN ('TEMPORARY_RESIDENCE', 'TEMPORARY_ABSENCE', 'TAM_TRU', 'TAM_VANG')`, [requestId]);
        if ((result?.rowCount ?? 0) > 0) {
            ...
            source = "tam_tru_vang";
            const ttvResult = await (0, db_1.query)(`SELECT
            ttv.id,
            CASE WHEN ttv.loai = 'tam_tru' THEN 'TEMPORARY_RESIDENCE' ELSE 'TEMPORARY_ABSENCE' END AS type,
            CASE ttv."trangThai"
              WHEN 'cho_duyet' THEN 'PENDING'
 ...
 */
router.post("/tam-tru-vang/requests/:id/reject", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const requestId = Number(req.params.id);
        const reviewerId = req.user.id;
        ...
        await (0, db_1.query)(`UPDATE requests SET status = 'REJECTED', "rejectionReason" = $1, "reviewedBy" = $2, "reviewedAt" = CURRENT_TIMESTAMP WHERE id = $3`, [reason.trim(), reviewerId, requestId]);
        return res.json({
            success: true,
...
function validateAddNewbornPayload(payload) {
...
function validateAddPersonPayload(payload, hasTargetHouseholdId = false) {
    ...
    const missingFields = requiredFields.filter((field) => person[field] === undefined ||
        person[field] === null ||
...
function validateTemporaryResidencePayload(payload) {
    const residenceData = payload.residence || payload;
    const personPayload = (payload?.person && typeof payload.person === "object" && payload.person) ||
        (payload?.nhanKhau &&
        ...
        (residenceData?.person &&
            ...
            residenceData.person) ||
        null;
    const nhanKhauIdRaw = residenceData?.nhanKhauId ??
        payload?.nhanKhauId ??
        payload?.personId ??
        null;
    const nhanKhauIdNum = nhanKhauIdRaw !== null &&
...
function validateTemporaryAbsencePayload(payload) {
    ...
    const personPayload = (payload?.person && typeof payload.person === "object" && payload.person) ||
        ...
        (absenceData?.person &&
            ...
            absenceData.person) ||
        null;
    const nhanKhauIdRaw = absenceData?.nhanKhauId ?? payload?.nhanKhauId ?? payload?.personId ?? null;
    const nhanKhauIdNum = nhanKhauIdRaw !== null &&
        nhanKhauIdRaw !== undefined &&
        nhanKhauIdRaw !== ""
        ? Number(nhanKhauIdRaw)
...
function validateSplitHouseholdPayload(payload) {
    if (!payload || typeof payload !== "object") {
        return "Thiếu dữ liệu tách hộ";
    }
    if (!Array.isArray(payload.selectedNhanKhauIds) ||
...
function validateUpdatePersonPayload(payload) {
    if (!payload || typeof payload !== "object") {
    ...
    const hasAnyCccdField = (typeof payload.cccd === "string" && payload.cccd.trim() !== "") ||
...
function validateRemovePersonPayload(payload) {
    if (!payload || typeof payload !== "object") {
...
function validateMoveOutPayload(payload) {
    if (!payload || typeof payload !== "object") {
 ...
 */
router.get("/requests/my", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireRole)(["nguoi_dan"]), async (req, res, next) => {
    try {
        const userId = req.user.id;
        const result = await (0, db_1.query)(`SELECT id, code, type, payload, status, "rejectionReason", "createdAt", "reviewedAt",
 ...
 */
router.get("/requests/:id", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const userId = req.user.id;
        const requestId = Number(req.params.id);
        if (!requestId) {
        ...
        const isRequester = await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 AND "requesterUserId" = $2`, [requestId, userId]);
        ...
        const result = await (0, db_1.query)(`SELECT r.*, u."fullName" as requesterName, u.username as requesterUsername, u.cccd as requesterCccd, ru."fullName" as reviewerName
       FROM requests r
       LEFT JOIN users u ON r."requesterUserId" = u.id
       ...
       WHERE r.id = $1`, [requestId]);
        ...
        let hoKhauLienQuan = null;
        if (row.targetHouseholdId) {
            const hk = await (0, db_1.query)(`SELECT id, "soHoKhau", "diaChi" FROM ho_khau WHERE id = $1`, [Number(row.targetHouseholdId)]);
            if ((hk?.rowCount ?? 0) > 0) {
                hoKhauLienQuan = {
        ...
        let nhanKhauLienQuan = null;
        if (row.targetPersonId) {
            const nk = await (0, db_1.query)(`SELECT id, "hoTen", cccd FROM nhan_khau WHERE id = $1`, [Number(row.targetPersonId)]);
            if ((nk?.rowCount ?? 0) > 0) {
                nhanKhauLienQuan = {
 ...
 */
router.get("/requests", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
 ...
 */
router.post("/requests/:id/approve", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const requestId = Number(req.params.id);
        const reviewerId = req.user.id;
        ...
        try {
            handlerResult = await approvalHandler(requestId, payload, reviewerId, finalHouseholdId, request.targetPersonId);
        }
        catch (e) {
        ...
        if (handlerResult && handlerResult.id) {
            await (0, db_1.query)(`UPDATE requests SET "targetPersonId" = $1 WHERE id = $2`, [
                handlerResult.id,
                requestId,
            ]);
        }
        // Update request status
        await (0, db_1.query)(`UPDATE requests
       ...
       WHERE id = $2`, [reviewerId, requestId]);
        ...
        const updatedReqRes = await (0, db_1.query)(`SELECT r.*, u."fullName" as requesterName, u.username as requesterUsername, u.cccd as requesterCccd, ru."fullName" as reviewerName
       FROM requests r
       LEFT JOIN users u ON r."requesterUserId" = u.id
       LEFT JOIN users ru ON r."reviewedBy" = ru.id
       WHERE r.id = $1`, [requestId]);
        ...
        if (pgCode === "23514") {
            return res.status(400).json({
                ...
                error: {
                    ...
                    message: "Ràng buộc dữ liệu không thỏa (check constraint). " + pgDetail,
                },
            });
        }
 ...
 */
router.post("/requests/:id/reject", auth_middleware_1.requireAuth, (0, auth_middleware_1.requireAnyTask)(["hokhau_nhankhau", "tamtru_tamvang"]), async (req, res, next) => {
    try {
        const requestId = Number(req.params.id);
        const reviewerId = req.user.id;
        const { reason } = req.body;
        ...
        const requestCheck = await (0, db_1.query)(`SELECT status FROM requests WHERE id = $1`, [requestId]);
       ...
       WHERE id = $3`, [reason.trim(), reviewerId, requestId]);
        return res.json({
            success: true,
...
function getApprovalHandler(type) {
    const handlers = {
        ...
        // legacy keys
        ["TAM_TRU"]: processTemporaryResidenceApproval,
        ["TAM_VANG"]: processTemporaryAbsenceApproval,
    };
 ...
 */
async function processAddNewbornApproval(requestId, payload, reviewerId, targetHouseholdId) {
    // Start transaction
    await (0, db_1.query)("BEGIN");
    // Lock request row to prevent concurrent approvals
    await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [requestId]);
    // Lock request row to prevent concurrent approvals
    await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [requestId]);
    try {
        const newborn = payload.newborn || payload;
        ...
        const householdCheck = await (0, db_1.query)(`SELECT id, "trangThai" FROM ho_khau WHERE id = $1`, [householdId]);
        ...
        if (cccd && cccd.trim() !== "") {
            const cccdCheck = await (0, db_1.query)(`SELECT id FROM nhan_khau WHERE cccd = $1`, [cccd.trim()]);
        ...
        const duplicateCheck = await (0, db_1.query)(`SELECT id FROM nhan_khau
       WHERE "hoKhauId" = $1 AND "hoTen" = $2 AND "ngaySinh" = $3 AND "quanHe" = 'con'`, [householdId, hoTen.trim(), ngaySinh]);
        ...
        const processedLyDoKhongCoCCCD = !cccd || String(cccd).trim() === "" ? "Mới sinh - chưa có CCCD" : null;
        ...
        const insertResult = await (0, db_1.query)(`INSERT INTO nhan_khau (
        ...
        await (0, db_1.query)(`INSERT INTO bien_dong (
      ...
      )`, [insertResult.rows[0].id, (0, date_1.getCurrentDateString)(), "Khai sinh", reviewerId]);
 ...
 */
async function processAddPersonApproval(requestId, payload, reviewerId, targetHouseholdId) {
    // Start transaction
    await (0, db_1.query)("BEGIN");
    try {
        const person = payload.person || payload;
        ...
        if (!householdId) {
            throw {
                ...
                message: "Cần chỉ định hộ khẩu để thêm nhân khẩu vào. Vui lòng liên hệ tổ trưởng.",
            };
        }
        const { hoTen, cccd, lyDoKhongCoCCCD, ngaySinh, gioiTinh, noiSinh, nguyenQuan, danToc, tonGiao, quocTich, quanHe, ngayDangKyThuongTru, diaChiThuongTruTruoc, ngheNghiep, noiLamViec, ghiChu, } = person;
        ...
        const duplicateCheck = await (0, db_1.query)(`SELECT id FROM nhan_khau
       WHERE "hoKhauId" = $1 AND "hoTen" = $2 AND "ngaySinh" = $3`, [householdId, hoTen.trim(), ngaySinh]);
        if ((duplicateCheck?.rowCount ?? 0) > 0) {
            throw {
        ...
        if (processedQuanHe === "chu_ho") {
            const existingChuHo = await (0, db_1.query)(`SELECT id FROM nhan_khau WHERE "hoKhauId" = $1 AND "quanHe" = 'chu_ho'`, [householdId]);
            if ((existingChuHo?.rowCount ?? 0) > 0) {
                throw {
                    code: "DUPLICATE_CHU_HO",
                    message: "Hộ khẩu đã có chủ hộ. Vui lòng sử dụng chức năng đổi chủ hộ.",
                };
            }
        }
        ...
        const processedGhiChu = normalizedCccd || lyDoKhongCoCCCD
            ? ghiChu
                ...
                : null
            : null;
        const processedNgayDangKyThuongTru = ngayDangKyThuongTru || (0, date_1.getCurrentDateString)();
        ...
        if (processedQuanHe === "chu_ho") {
            await (0, db_1.query)(`UPDATE ho_khau SET "chuHoId" = $1 WHERE id = $2`, [
                created.id,
                householdId,
            ]);
        }
        ...
        if (cccd && cccd.trim() !== "") {
         ...
         WHERE role = 'nguoi_dan'
           ...
           AND normalize_cccd(username) = normalize_cccd($2)`, [created.id, cccd.trim()]);
        }
        await (0, db_1.query)("COMMIT");
        return created;
    }
...
async function processUpdatePersonApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    await (0, db_1.query)("BEGIN");
    try {
        await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [
            requestId,
        ]);
        ...
        const current = await (0, db_1.query)(`SELECT id, "hoKhauId", "quanHe", "trangThai", "cccd" FROM nhan_khau WHERE id = $1`, [nhanKhauId]);
        ...
        const hasAnyCccdField = payloadCccd !== "" || payloadNgayCap !== "" || payloadNoiCap !== "";
        ...
        const allowed = [
            ...
            { key: "biDanh", col: '"biDanh"', normalize: (v) => String(v).trim() },
            {
                key: "ngaySinh",
                col: '"ngaySinh"',
                normalize: (v) => (0, date_1.normalizeDateOnly)(v),
            },
        ...
        const lyDo = String(payload?.lyDo || payload?.reason || "Cập nhật thông tin").trim();
        ...
        await (0, db_1.query)(`UPDATE nhan_khau
       SET ${setClauses.join(", ")},
...
async function processRemovePersonApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    await (0, db_1.query)("BEGIN");
    try {
        await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [
            requestId,
        ...
        const person = await (0, db_1.query)(`SELECT id, "hoKhauId", "quanHe", "trangThai" FROM nhan_khau WHERE id = $1`, [nhanKhauId]);
        if ((person?.rowCount ?? 0) === 0) {
            throw { code: "PERSON_NOT_FOUND", message: "Nhân khẩu không tồn tại" };
        }
        const row = person.rows[0];
        ...
        const noteLine = `Xoá nhân khẩu (${(0, date_1.getCurrentDateString)()}): ${lyDo}`;
        await (0, db_1.query)(`UPDATE nhan_khau
       SET "trangThai" = 'deleted',
           ...
           END,
           "updatedAt" = CURRENT_TIMESTAMP
       WHERE id = $1`, [nhanKhauId, noteLine]);
        await (0, db_1.query)("COMMIT");
 ...
 */
async function processTemporaryResidenceApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    // Start transaction
    await (0, db_1.query)("BEGIN");
    try {
        await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [
        ...
        const reqMeta = await (0, db_1.query)(`SELECT "requesterUserId", "targetHouseholdId" FROM requests WHERE id = $1`, [requestId]);
        const requesterUserId = (reqMeta?.rowCount ?? 0) > 0
            ? reqMeta.rows[0].requesterUserId ?? null
            : null;
        const effectiveHouseholdId = targetHouseholdId
            ? Number(targetHouseholdId)
        ...
        let nhanKhauId = null;
        if (targetPersonId)
            nhanKhauId = Number(targetPersonId);
        ...
        if (!diaChi || String(diaChi).trim() === "") {
            const household = await (0, db_1.query)(`SELECT "soHoKhau" FROM ho_khau WHERE id = $1`, [effectiveHouseholdId]);
            const soHoKhau = household?.rows?.[0]?.soHoKhau;
            if (!soHoKhau) {
                throw {
                    code: "VALIDATION_ERROR",
        ...
        if (!nhanKhauId && personPayload) {
            ...
            if (!hoTen || !ngaySinh || !gioiTinh || !noiSinh || !quanHe) {
                throw {
                    code: "VALIDATION_ERROR",
                    message: "Thiếu thông tin nhân khẩu bắt buộc: hoTen, ngaySinh, gioiTinh, noiSinh, quanHe",
                };
            }
            // If CCCD provided and exists, reuse only if same household
            if (cccd) {
        ...
        const nhanKhauCheck = await (0, db_1.query)(`SELECT id, "hoTen", "trangThai" FROM nhan_khau WHERE id = $1`, [nhanKhauId]);
        ...
        const existingCheck = await (0, db_1.query)(`SELECT id FROM tam_tru_vang
       WHERE "nhanKhauId" = $1 AND "trangThai" IN ('cho_duyet', 'da_duyet', 'dang_thuc_hien')
       AND (denNgay IS NULL OR denNgay >= CURRENT_DATE)`, [nhanKhauId]);
        ...
        await (0, db_1.query)(`UPDATE nhan_khau SET "trangThai" = 'tam_tru' WHERE id = $1`, [
            nhanKhauId,
        ]);
        await (0, db_1.query)("COMMIT");
 ...
 */
async function processTemporaryAbsenceApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    // Start transaction
    await (0, db_1.query)("BEGIN");
    try {
        await (0, db_1.query)(`SELECT id FROM requests WHERE id = $1 FOR UPDATE`, [
        ...
        const effectiveHouseholdId = targetHouseholdId
            ...
            : (reqMeta?.rowCount ?? 0) > 0 && reqMeta.rows[0].targetHouseholdId
                ? Number(reqMeta.rows[0].targetHouseholdId)
                : null;
        const absenceData = payload?.absence && typeof payload.absence === "object"
            ? payload.absence
            : payload;
       ...
       AND (denNgay IS NULL OR denNgay >= CURRENT_DATE)`, [nhanKhauId]);
        ...
        await (0, db_1.query)(`UPDATE nhan_khau SET "trangThai" = 'tam_vang' WHERE id = $1`, [
            nhanKhauId,
        ]);
        await (0, db_1.query)("COMMIT");
        return { id: nhanKhauId, tamTruVangId: insertResult.rows[0]?.id };
...
async function processSplitHouseholdApproval(requestId, payload, reviewerId, targetHouseholdId) {
    ...
    try {
        ...
        const expectedDate = payload?.expectedDate || (0, date_1.getCurrentDateString)();
        // Kiểm tra hộ khẩu gốc
        const originalHousehold = await (0, db_1.query)(`SELECT id, "chuHoId", "trangThai", "soHoKhau", "diaChi" FROM ho_khau WHERE id = $1`, [originalHouseholdId]);
        ...
        const selectedPeople = await (0, db_1.query)(`SELECT id, "hoKhauId", "quanHe" FROM nhan_khau WHERE id = ANY($1::int[])`, [selectedIds]);
        ...
        const newHouseholdResult = await (0, db_1.query)(`INSERT INTO ho_khau ("soHoKhau", "diaChi", "ngayCap", "trangThai", "ghiChu", "chuHoId")
       VALUES (generate_ho_khau_code(), $1, $2, 'inactive', $3, NULL)
       RETURNING id, "soHoKhau"`, [newAddress, expectedDate, payload?.reason || null]);
       ...
       SET "hoKhauId" = $1,
           ...
           "updatedAt" = CURRENT_TIMESTAMP
       WHERE id = ANY($4::int[])`, [newHouseholdId, newChuHoId, expectedDate, selectedIds]);
        // Kích hoạt hộ khẩu mới và gán chủ hộ
        await (0, db_1.query)(`UPDATE ho_khau
       SET "chuHoId" = $1, "trangThai" = 'active', "ngayCap" = COALESCE("ngayCap", $2)
       WHERE id = $3`, [newChuHoId, expectedDate, newHouseholdId]);
        ...
        if (chuHoMoved) {
            ...
            const remaining = await (0, db_1.query)(`SELECT id FROM nhan_khau WHERE "hoKhauId" = $1 ORDER BY id`, [originalHouseholdId]);
            if ((remaining?.rowCount ?? 0) > 0) {
                const replacementFromPayloadRaw = payload?.oldHouseholdNewChuHoId ?? payload?.oldChuHoIdReplacement;
                ...
                if (!replacementFromPayload) {
                    throw {
                        code: "VALIDATION_ERROR",
                        message: "Khi tách chủ hộ ra khỏi hộ khẩu gốc, vui lòng chọn chủ hộ thay thế cho hộ khẩu gốc",
                    };
                }
                const newHeadId = replacementFromPayload;
                const isValidRemaining = remaining.rows.some((r) => Number(r.id) === Number(newHeadId));
                if (!isValidRemaining) {
                    throw {
                        code: "VALIDATION_ERROR",
                        message: "Vui lòng chọn chủ hộ thay thế hợp lệ cho hộ khẩu gốc khi tách chủ hộ",
                    };
                }
                await (0, db_1.query)(`UPDATE nhan_khau SET "quanHe" = 'chu_ho' WHERE id = $1`, [
                    newHeadId,
                ]);
                await (0, db_1.query)(`UPDATE ho_khau SET "chuHoId" = $1 WHERE id = $2`, [
                    newHeadId,
                ...
                await (0, db_1.query)(`UPDATE ho_khau SET "chuHoId" = NULL, "trangThai" = 'inactive' WHERE id = $1`, [originalHouseholdId]);
            }
        }
        // Ghi lịch sử: hộ gốc đã tách các thành viên sang hộ mới
        const movedMembers = await (0, db_1.query)(`SELECT id, "hoTen" FROM nhan_khau WHERE id = ANY($1::int[])`, [selectedIds]);
        await (0, db_1.query)(`INSERT INTO lich_su_thay_doi (bang, "banGhiId", "hanhDong", truong, "noiDungCu", "noiDungMoi", "nguoiThucHien")
...
async function processDeceasedApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    await (0, db_1.query)("BEGIN");
    try {
        const nhanKhauId = targetPersonId || payload?.nhanKhauId || payload?.personId;
        if (!nhanKhauId) {
        ...
        const person = await (0, db_1.query)(`SELECT id, "trangThai", "hoKhauId" FROM nhan_khau WHERE id = $1`, [nhanKhauId]);
        if ((person?.rowCount ?? 0) === 0) {
            throw { code: "PERSON_NOT_FOUND", message: "Nhân khẩu không tồn tại" };
        }
        ...
        const ngayMat = (0, date_1.normalizeDateOnly)(payload?.ngayMat || payload?.dateOfDeath || (0, date_1.getCurrentDateString)());
...
async function processMoveOutApproval(requestId, payload, reviewerId, targetHouseholdId, targetPersonId) {
    await (0, db_1.query)("BEGIN");
    try {
        const nhanKhauId = targetPersonId || payload?.nhanKhauId || payload?.personId;
        if (!nhanKhauId) {
        ...
        const ngayChuyen = (0, date_1.normalizeDateOnly)(payload?.ngayChuyen ||
            payload?.ngayDi ||
            payload?.ngayThucHien ||
            (0, date_1.getCurrentDateString)());
        const lyDo = (payload?.lyDo ||
            payload?.reason ||
            payload?.noiDung ||
            "Chuyển đi").toString();
