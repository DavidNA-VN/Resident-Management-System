...
router.get(
  "/dashboard",
  requireAuth,
  requireRole(["admin", "to_truong", "to_pho", "can_bo"]),
  async (req, res) => {
    try {
      ...
      const [countsRes, quickRes, recentRes, notiRes] = await Promise.all([
        query(countsQuery),
        query(quickStatsQuery),
        query(recentActivitiesQuery),
        userId
          ? query(notificationsQuery, [userId])
      ...
      const hoKhauChange =
        asNumber(row.ho_khau_created_this_month) -
        asNumber(row.ho_khau_created_last_month);
      const nhanKhauChange =
        asNumber(row.nhan_khau_created_this_month) -
        asNumber(row.nhan_khau_created_last_month);
      const bienDongChange =
        asNumber(row.bien_dong_this_month) - asNumber(row.bien_dong_last_month);
      const phanAnhChange =
        asNumber(row.phan_anh_created_this_month) -
      ...
      return res.json({
        ...
        recentActivities: (recentRes.rows || []).map((r: any, idx: number) => ({
          id: idx + 1,
          type: String(r.type),
        ...
        notifications: (notiRes.rows || []).map((n: any) => ({
          id: Number(n.id),
          type: String(n.type),
