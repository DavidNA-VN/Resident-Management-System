...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const hoKhauResult = await query(`SELECT * FROM ho_khau WHERE id = $1`, [
        hoKhauId,
      ]);
  ...
  async (req, res, next) => {
    try {
      ...
      const userRow = await query(
        `SELECT "personId", username FROM users WHERE id = $1`,
        [userId]
      );
      ...
      console.log(
        "[GET /citizen/me/households] userId:",
        userId,
        "personId:",
        personId,
        "username:",
